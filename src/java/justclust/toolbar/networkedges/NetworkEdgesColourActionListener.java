...
public class NetworkEdgesColourActionListener implements ActionListener {

    @Override
    public void actionPerformed(ActionEvent actionEvent) {

        if (actionEvent.getActionCommand().equals("OK")) {
            NetworkEdgesTableCellEditor.classInstance.okButtonClicked = true;
            NetworkEdgesColourJDialog.classInstance.dispose();
        }

        if (actionEvent.getActionCommand().equals("Cancel")) {
            NetworkEdgesTableCellEditor.classInstance.okButtonClicked = false;
            NetworkEdgesColourJDialog.classInstance.dispose();
        }

        if (actionEvent.getActionCommand().equals("Random")) {
            Random random = new Random();
            NetworkEdgesColourJDialog.classInstance.jColorChooser.setColor(random.nextInt(256), random.nextInt(256), random.nextInt(256));
        }
