    ...
    public void updateTable() {
        ...
        if (data.graphShown) {
            ...
            if (data.networkClusters.size() >= 1) {
                tableData[0][3] = data.networkClusters.get(0).nodes.get(0).nodeGraphicalAttributes.colour;
            }
            LOOP:
            for (Cluster cluster : data.networkClusters) {
                for (Node node : cluster.nodes) {
                    if (!node.nodeGraphicalAttributes.colour.equals(data.networkClusters.get(0).nodes.get(0).nodeGraphicalAttributes.colour)) {
            ...
            if (comparisonEdge != null) {
                tableData[0][4] = comparisonEdge.edgeGraphicalAttributes.colour;
                LOOP:
                for (Cluster cluster : data.networkClusters) {
                    for (Node node : cluster.nodes) {
                        for (Edge edge : node.edges) {
                            if (edge.node1 == node
                                    ...
                                    && !edge.edgeGraphicalAttributes.colour.equals(comparisonEdge.edgeGraphicalAttributes.colour)) {
                ...
                for (int j = 0; j < maxClusterSize + 6; j++) {
                    switch (j) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            tableData[i + 1][j] = data.networkClusters.get(i).nodes.get(0).nodeGraphicalAttributes.colour;
                            for (Node node : data.networkClusters.get(i).nodes) {
                                if (!node.nodeGraphicalAttributes.colour.equals(data.networkClusters.get(i).nodes.get(0).nodeGraphicalAttributes.colour)) {
                        ...
                        case 4:
                            ...
                            if (comparisonEdge != null) {
                                tableData[i + 1][j] = comparisonEdge.edgeGraphicalAttributes.colour;
                                LOOP:
                                for (Node node : data.networkClusters.get(i).nodes) {
                                    for (Edge edge : node.edges) {
                                        if (edge.node1 == node
            ...
            columnNames[0] = "Node Amount";
            for (int i = 0; i < maxClusterSize; i++) {
                columnNames[i + 1] = "Node " + (i + 1);
            }

            tableData = new Object[data.networkClusters.size() + 1][maxClusterSize + 1];
            tableData[0][0] = "";
            ...
            // about the cluster
            for (int i = 0; i < data.networkClusters.size(); i++) {
                for (int j = 0; j < maxClusterSize + 1; j++) {
                    switch (j) {
                        case 0:
                            tableData[i + 1][j] = data.networkClusters.get(i).nodes.size();
                            break;
                        default:
                            if (data.networkClusters.get(i).nodes.size() > j - 1) {
                                tableData[i + 1][j] = data.networkClusters.get(i).nodes.get(j - 1).label;
                            } else {
                                tableData[i + 1][j] = "";
                            }
                            break;
    ...
    public void setValueAt(Object value, int row, int col) {
        ...
        if (data.graphShown) {
            ...
            if (col == 1) {
                ...
                if (NetworkClustersJDialog.classInstance.networkClustersListSelectionModel.isSelectedIndex(0)) {
                    ...
                    for (int i = 0; i < data.networkClusters.size(); i++) {
                        ...
                        for (Node node : data.networkClusters.get(i).nodes) {
                            node.nodeGraphicalAttributes.visible = (boolean) value;
                        ...
                        for (Node node : cluster.nodes) {
                            for (Edge edge : node.edges) {
                                if (edge.node1.cluster == edge.node2.cluster) {
                                    edge.edgeGraphicalAttributes.visible = (boolean) value;
                                    for (Edge otherVersion : edge.edgeGraphicalAttributes.otherVersions) {
            ...
            if (col == 3 && NetworkClustersTableCellEditor.classInstance.okButtonClicked) {
                ...
                if (NetworkClustersJDialog.classInstance.networkClustersListSelectionModel.isSelectedIndex(0)) {
                    ...
                    for (int i = 0; i < data.networkClusters.size(); i++) {
                        ...
                        fireTableCellUpdated(i + 1, col);
                        for (Node node : data.networkClusters.get(i).nodes) {
                            node.nodeGraphicalAttributes.colour = (Color) value;
                            for (Node otherVersion : node.nodeGraphicalAttributes.otherVersions) {
                ...
                // this cell should be white (the default Node colour).
                if (data.networkClusters.size() >= 1) {
                    tableData[0][col] = data.networkClusters.get(0).nodes.get(0).nodeGraphicalAttributes.colour;
                }
                LOOP:
                for (Cluster cluster : data.networkClusters) {
                    for (Node node : cluster.nodes) {
            ...
            if (col == 4 && NetworkClustersTableCellEditor.classInstance.okButtonClicked) {
                ...
                if (NetworkClustersJDialog.classInstance.networkClustersListSelectionModel.isSelectedIndex(0)) {
                    ...
                    for (int i = 0; i < data.networkClusters.size(); i++) {
                        ...
                        for (Node node : data.networkClusters.get(i).nodes) {
                            for (Edge edge : node.edges) {
                                if (edge.node1.cluster == edge.node2.cluster) {
                                    edge.edgeGraphicalAttributes.colour = (Color) value;
                                    for (Edge otherVersion : edge.edgeGraphicalAttributes.otherVersions) {
                ...
                if (comparisonEdge != null) {
                    tableData[0][col] = comparisonEdge.edgeGraphicalAttributes.colour;
                    LOOP:
                    for (Cluster cluster : data.networkClusters) {
                        for (Node node : cluster.nodes) {
                            for (Edge edge : node.edges) {
