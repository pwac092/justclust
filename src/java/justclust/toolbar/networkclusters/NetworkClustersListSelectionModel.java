 ...
 */
public class NetworkClustersListSelectionModel implements ListSelectionModel, Cloneable, Serializable {
     ...
     * empty array if no list selection listeners are currently registered
     *
    ...
    /**
     * Notifies
     * <code>ListSelectionListeners</code> that the value of the selection, in
     * the closed interval
     ...
     * <code>DefaultListSelectionModel</code> instance
     ...
     * <code>leadAnchorNotificationEnabled</code> flag. When
     * <code>leadAnchorNotificationEnabled</code> is true the model generates
     * notification events with bounds that cover all the changes to the
     * selection plus the changes to the lead and anchor indices. Setting the
     * flag to false causes a narrowing of the event's bounds to include only
     * the elements that have been selected or deselected since the last change.
     * Either way, the model continues to maintain the lead and anchor variables
     * internally. The default is true.
     * <p>
    ...
    private void changeSelection(int clearMin, int clearMax,
        ...
        // when the visibility or colour of a Cluster is changed and the Cluster
        // is already selected in the table, no selection changes should occur.
        // this will mean that all previously selected Clusters will remain
        // selected.
        if (NetworkClustersJDialog.classInstance.networkClustersDialogJTable.getSelectedColumn() >= 1
                && NetworkClustersJDialog.classInstance.networkClustersDialogJTable.getSelectedColumn() <= 4
                && setMin == setMax
                && isSelectedIndex(setMin)) {
            return;
        }
    ...
    public Object clone() throws CloneNotSupportedException {
        NetworkClustersListSelectionModel clone = (NetworkClustersListSelectionModel) super.clone();
     ...
     * <code>mouseDragged</code> method of a UI class to extend a selection.
     *
     * @see #getLeadSelectionIndex
     * @see #setAnchorSelectionIndex
     */
