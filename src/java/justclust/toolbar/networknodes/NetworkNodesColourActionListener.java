...
public class NetworkNodesColourActionListener implements ActionListener {

    @Override
    public void actionPerformed(ActionEvent actionEvent) {

        if (actionEvent.getActionCommand().equals("OK")) {
            NetworkNodesTableCellEditor.classInstance.okButtonClicked = true;
            NetworkNodesColourJDialog.classInstance.dispose();
        }

        if (actionEvent.getActionCommand().equals("Cancel")) {
            NetworkNodesTableCellEditor.classInstance.okButtonClicked = false;
            NetworkNodesColourJDialog.classInstance.dispose();
        }

        if (actionEvent.getActionCommand().equals("Random")) {
            Random random = new Random();
            NetworkNodesColourJDialog.classInstance.jColorChooser.setColor(random.nextInt(256), random.nextInt(256), random.nextInt(256));
        }
