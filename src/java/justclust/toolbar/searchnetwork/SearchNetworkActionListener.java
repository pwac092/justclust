        ...
        if (actionEvent.getActionCommand().equals("Search")) {
            ...
            if (data.graphShown) {
                ...
                for (Edge edge : data.networkEdges) {
                    ...
                    // graphically
                    if (edge.edgeGraphicalAttributes.label == null && SearchNetworkJDialog.classInstance.searchJTextField.getText().equals("")
                            || edge.edgeGraphicalAttributes.label != null && edge.edgeGraphicalAttributes.label.toLowerCase().contains(SearchNetworkJDialog.classInstance.searchJTextField.getText().toLowerCase())) {
                        if (!foundEdges.contains(edge)) {
                            foundEdges.add(edge);
                        }
                    }
            ...
            if (data.graphShown) {
                ...
                for (int i = 0; i < foundEdges.size(); i++) {
                    tableData[i][0] = foundEdges.get(i).edgeGraphicalAttributes.label;
                ...
                tableData = new String[foundEdges.size()][columnNames.length];
                for (int i = 0; i < foundEdges.size(); i++) {
                    tableData[i][0] = foundEdges.get(i).node1.label;
                    tableData[i][1] = foundEdges.get(i).node2.label;
                    tableData[i][2] = new DecimalFormat("#.#####").format(foundEdges.get(i).weight);;
                }
            }
            ...
            if (data.graphShown) {
                ...
                columnNames = new String[maxClusterSize];
                for (int i = 0; i < maxClusterSize; i++) {
                    columnNames[i] = "Node " + i;
                }
                tableData = new String[foundClusters.size()][columnNames.length];
                for (int i = 0; i < foundClusters.size(); i++) {
                    for (int j = 0; j < maxClusterSize; j++) {
                        if (foundClusters.get(i).nodes.size() > j) {
                            tableData[i][j] = foundClusters.get(i).nodes.get(j).label;
                        } else {
                            tableData[i][j] = "";
                        }
                    }
