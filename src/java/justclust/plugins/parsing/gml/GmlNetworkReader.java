...
//    private Map<String, CyNode> nodeIDMap;
    ...
    private Vector<Double> nodeXCoordinates;
    private Vector<Double> nodeYCoordinates;
//    private Vector<CyEdge> edgeNames;
    private Vector<Edge> edgeNames;
...
//    }
    protected void initializeStructures() {
        ...
        nodeXCoordinates = new Vector<Double>();
        nodeYCoordinates = new Vector<Double>();
    }
...
//        nodeIDMap = new HashMap<String, CyNode>(nodes.size());
        nodeIDMap = new HashMap<String, Node>(nodes.size());
//        Map<Object, CyNode> nMap = getNodeMap();
        Map<Object, Node> nMap = new HashMap<Object, Node>();
...
//                    node = network.addNode();
                node = new Node();
                GmlFileParser.classInstance.networkNodes.add(node);
//                }
...
//                final CyNode node1 = nodeIDMap.get(sourceName);
                final Node node1 = nodeIDMap.get(sourceName);
//                final CyNode node2 = nodeIDMap.get(targetName);
                final Node node2 = nodeIDMap.get(targetName);
//                CyEdge edge = network.addEdge(node1, node2,
//                        isDirected.booleanValue());
                Edge edge = new Edge();
                edge.node1 = node1;
                edge.node2 = node2;
                GmlFileParser.classInstance.networkEdges.add(edge);
    ...
    protected void readNode(final List<KeyValue> list) {
        ...
        for (KeyValue keyVal : list) {
            if (keyVal.key.equals(ID)) {
                ...
                xCoordinate = readNodeXCoordinate((List<KeyValue>) keyVal.value);
                yCoordinate = readNodeYCoordinate((List<KeyValue>) keyVal.value);
            } else if (!keyVal.key.equals(GRAPHICS)
...
//                GMLParser.printList(list, stringWriter);
            ...
            nodeXCoordinates.add(xCoordinate);
            nodeYCoordinates.add(yCoordinate);
        }
    }

    double readNodeXCoordinate(List<KeyValue> list) {
        for (KeyValue keyVal : list) {
            if (keyVal.key.equals("x")) {
                return (Double) keyVal.value;
            }
        }
        return 0;
    }

    double readNodeYCoordinate(List<KeyValue> list) {
        for (KeyValue keyVal : list) {
